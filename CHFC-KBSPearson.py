...
LastEditTime: 2023-06-21 11:17:04
Description: 
'''
...
if __name__ == '__main__':
    ...
    for pa in file_list:
        ...
        for train_index,test_index in KF.split(data):
            ...
            for item in Noise:
                if Noise.count(item) !=1:
                    print("the %s has found %s" %(item,Noise.count(item)))
            ...
            # print(SimOriginal)
            # knnOriginal = KNN(train)
            # sim_save_path = savepath+fileName+str(number)+'SimOriginal.txt'
            # print(sim_save_path)
            # f=open(sim_save_path,'r')
            # SimOriginal = eval(f.read())
            # f.close()
            ...
            # f.close()
            # with open(sim_save_path, 'a') as f:
            #     f.write('\n'+str(SimOriginal))   
            # print(userSim)

            print('对可能的噪声评分进行预测')
            prePossibleNoise = knnOriginal.predict(Noise, SimOriginal, Kneighbor)
            # myset = set(prePossibleNoise)
            for item in prePossibleNoise:
                if prePossibleNoise.count(item) !=1:
                    print("the %s has found %s" %(item,prePossibleNoise.count(item)))
            ...
            # f.write(str(itemSimCorrections)) 
            # f.close
